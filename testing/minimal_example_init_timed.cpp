...
int main()
{
	...
	for(int i = 0; i!=100; i++)
	{
		...
		declare_with_name<std::vector<double>>("vec",local_name_set);
		declare<double,std::unique_ptr<double>,std::shared_ptr<double>,std::optional<double>>(local_name_set);


		//use imports a function set. an empty string means to not use a namespace
		env.functions.use<core>("").use<cpp_core>("").use<terminal>()
			...
			<< fs_name<types>(local_name_set) << fs_functs<types>(local_name_set)
	...
	std::cout << "time: " << double(std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count())/100.0 << "microseconds average";
