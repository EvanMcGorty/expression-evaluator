...
namespace expr
{
	namespace impl
	{
		...
		struct core
		{
			static any_value swap(std::vector<stack_elem>& a)
			{
				if (a.size() == 2 && (a[0].is_nullval() || a[0]->has_value()) && (a[1].is_nullval() || a[1]->has_value()))
				{
					...
					return any_value::make<void_object>(); //indicates a successful function call even though the return type is void
				}
				else
				{
					return any_value::make_nullval();
				}
			}

			static any_value first(std::vector<stack_elem>& a)
			{
			...
			static any_value last(std::vector<stack_elem>& a)
			{
		...
		struct cpp_core
		{
			static any_value strengthen(std::vector<stack_elem>& a)
			{
			...
			static any_value to_string(std::vector<stack_elem>& a)
			{
				if (a.size() == 1 && !a[0].is_nullval())
				{
					return any_value::make<object_of<std::string>>(a[0]->convert_into_string());
				}
				else
				{
					return any_value::make_nullval();
			...
			static any_value drop(std::vector<stack_elem>& a)
			{
				for (auto& it : a)
				{
					if (!it.is_nullval())
			...
			static any_value clone(std::vector<stack_elem>& a)
			{
				if (a.size() == 1 && !a[0].is_nullval() && a[0]->is_object())
				{
			...
			static any_value take(std::vector<stack_elem>& a)
			{
				if (a.size() == 1 && !a[0].is_nullval() && a[0]->is_object())
				{
		...
		{
			static any_value from_list(std::vector<stack_elem>& a)
			{
				std::vector<t> ret;
				for (auto& v : a)
				{
				...
				return any_value::make<object_of<std::vector<t>>>(std::move(ret));
			}
